...
export default function InputDataPanel({
  ...
  return (
    <div className={`border-t border-border bg-surface p-3 shrink-0 ${className}`}>
      ...
      <textarea
        className={`w-full h-[calc(100%-1.8rem)] resize-none rounded-md border bg-white p-2 text-sm font-code outline-none focus:ring-1 transition-colors disabled:bg-slate-50 disabled:text-slate-400 ${
        ...
        disabled={disabled}
      />
      {error && <p className="mt-1 text-[10px] text-danger leading-relaxed">{error}</p>}
    </div>
  )
}
