...
import EdgeRenderer from '@/scene/graphics/renderers/EdgeRenderer'
...
describe('EdgeRenderer', () => {
  it('renders a straight path between two resolved anchors', () => {
    ...
    const { container } = render(<svg><EdgeRenderer edge={edge()} scene={scene} /></svg>)
  ...
  it('renders a directed arrow marker and a label', () => {
    ...
    const { container } = render(
      <svg><EdgeRenderer edge={edge({ directed: true, label: 'w=5' })} scene={scene} /></svg>,
    )
    const path = container.querySelector('path')
  ...
  it('uses dependency marker for dependency variant', () => {
    const scene = sceneWith([node('a', 0, 0), node('b', 200, 0)])
    const { container } = render(
      <svg><EdgeRenderer edge={edge({ directed: true, variant: 'dependency' })} scene={scene} /></svg>,
    )
  ...
  it('uses color-matched trajectory marker for dashed+curved success edges', () => {
    ...
    const { container } = render(<svg><EdgeRenderer edge={e} scene={scene} /></svg>)
    const path = container.querySelector('path')
  ...
  it('uses danger and primary trajectory markers', () => {
    const scene = sceneWith([node('a', 0, 0), node('b', 200, 50)])
    const danger = render(
      <svg><EdgeRenderer edge={edge({ directed: true, state: { color: 'danger' }, style: { dashed: true, curved: true } })} scene={scene} /></svg>,
    )
    expect(danger.container.querySelector('path')?.getAttribute('marker-end')).toBe('url(#sceneTrajectoryDanger)')
    const primary = render(
      <svg><EdgeRenderer edge={edge({ directed: true, style: { dashed: true, curved: true, color: 'primary' } })} scene={scene} /></svg>,
    )
  ...
  it('computes a close-points curved dip', () => {
    ...
    const { container } = render(
      <svg><EdgeRenderer edge={edge({ style: { curved: true } })} scene={scene} /></svg>,
    )
  ...
  it('computes a vertical curved side arc honoring port side', () => {
    ...
    const { container } = render(
      <svg><EdgeRenderer edge={edge({ from: { entityId: 'a', portId: 'right' }, to: { entityId: 'b' }, style: { curved: true } })} scene={scene} /></svg>,
    )
    expect(container.querySelector('path')?.getAttribute('d')).toContain('Q ')
  })
