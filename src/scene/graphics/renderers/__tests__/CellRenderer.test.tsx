...
import CellRenderer from '@/scene/graphics/renderers/CellRenderer'
...
describe('CellRenderer', () => {
  it('renders a rect and value text', () => {
    const { container } = render(<svg><CellRenderer cell={cell()} /></svg>)
  ...
  it('does NOT draw the array index label itself (axis layer owns it so it stays pinned during swaps)', () => {
    const { container } = render(<svg><CellRenderer cell={cell({ id: 'arr_0', col: 0, value: 7 })} /></svg>)
    const texts = Array.from(container.querySelectorAll('text')).map(visibleText)
  ...
  it('renders row,col index label when row is present', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ id: 'm_0_0', row: 1, col: 2 })} /></svg>,
    )
    const texts = Array.from(container.querySelectorAll('text')).map(visibleText)
  ...
  it('returns null for empty_placeholder role', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { role: 'empty_placeholder' } })} /></svg>,
    )
  ...
  it('returns null for skipped id prefixes', () => {
    for (const id of ['mathvar_x', 'geo_1', 'auto_2', 'prob_3']) {
      const { container } = render(<svg><CellRenderer cell={cell({ id })} /></svg>)
      expect(container.querySelector('rect')).toBeNull()
    }
  })
  ...
  it('renders header role as plain text only', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { role: 'header' }, value: 'i' })} /></svg>,
    )
    expect(container.querySelector('rect')).toBeNull()
  ...
  it('draws a current ring for current/active role', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { role: 'current' } })} /></svg>,
    )
  ...
  it('uses window palette for window role', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { role: 'window' } })} /></svg>,
    )
  ...
  it('uses danger text color for conflict role', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { role: 'conflict' } })} /></svg>,
    )
  ...
  it('maps legacy color names to semantic palette', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { color: 'success' } })} /></svg>,
    )
  ...
  it('hides col label for dedicated-structure prefixes', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ id: 'stack_0', col: 0 })} /></svg>,
    )
    const texts = Array.from(container.querySelectorAll('text')).map(visibleText)
  ...
  it('hides col label for string char cells', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ id: 's_0_3', value: 'a', col: 3 })} /></svg>,
    )
    const texts = Array.from(container.querySelectorAll('text')).map(visibleText)
  ...
  it('truncates long values and nests original in a title', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ value: 'a-very-long-value-that-overflows', size: { width: 30, height: 30 } })} /></svg>,
    )
  ...
  it('applies pulse class and opacity', () => {
    const { container } = render(
      <svg><CellRenderer cell={cell({ state: { pulse: true, opacity: 0.4 } })} /></svg>,
    )
