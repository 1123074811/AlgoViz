...
import NodeRenderer from '@/scene/graphics/renderers/NodeRenderer'
...
describe('NodeRenderer', () => {
  it('renders a circle for graph/tree/union_find variants with value text', () => {
    const { container } = render(
      <svg><NodeRenderer node={baseNode()} /></svg>,
    )
  ...
  it('renders an active ring when role is active/visited/current', () => {
    const { container } = render(
      <svg><NodeRenderer node={baseNode({ state: { role: 'active', color: 'primary' } })} /></svg>,
    )
  ...
  it('applies pulse class and opacity', () => {
    const { container } = render(
      <svg><NodeRenderer node={baseNode({ state: { pulse: true, opacity: 0.5 } })} /></svg>,
    )
  ...
  it('renders extra circle fields as secondary labels', () => {
    ...
    const { container } = render(<svg><NodeRenderer node={node} /></svg>)
