...
describe('parseInputData', () => {
  ...
  it('keeps single non-root LeetCode assignments as objects', () => {
    const parsed = parseInputData('intervals = [[1,3],[1,4],[2,5],[3,5]]')

    expect(parsed.valid).toBe(true)
    expect(parsed.kind).toBe('array')
    expect(parsed.summary).toBe('数组 [4 个元素]')
    expect(parsed.value).toEqual({ intervals: [[1, 3], [1, 4], [2, 5], [3, 5]] })
  })
