...
- 必须对完整 input 执行原算法并得到真实结果；禁止为控制动画长度而截断输入、提前 return、break、限制搜索深度或只计算代表性分支。
- Builder 会在事件预算耗尽后自动停止记录教学事件，但你的循环/递归必须继续完整计算，并最终调用 \`b.result(realResult)\`；不要读取或自行维护动画预算。
