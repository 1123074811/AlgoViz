...
export function generateHeapOperations(arr: number[]): AnimationScript {
  ...
  for (const v of data) {
    ...
    while (i > 0) {
      ...
      steps[steps.length - 1].events = [{ type: 'tree.compare', nodeId: String(p), value: heap[p], result: 'greater' }]
    ...
    steps[steps.length - 1].events = [{ type: 'tree.insert', parentId: String(Math.floor((heap.length - 2) / 2)), node: { id: String(heap.length - 1), value: v } }]
  }
  ...
  while (true) {
    ...
    steps[steps.length - 1].events = [{ type: 'tree.visit', nodeId: String(i) }]
  ...
  return { algorithm: 'heap_ds', complexity: { time: { best: 'O(log n)', average: 'O(log n)', worst: 'O(log n)' }, space: 'O(n)' }, presentation: { engine: 'scene', module: 'tree' }, initialState: { type: 'array', data: heap }, steps: steps as AnimationScript['steps'] }
}
