...
export function generateHashTable(): AnimationScript {
  ...
  steps[0].events = [{ type: 'array.create', values: new Array(8).fill(0) }]
  ...
  for (const [k, v] of pairs) {
    ...
    steps[steps.length - 1].events = [{ type: 'array.compare', indices: [idx, idx] }]
  }
  ...
  steps[steps.length - 1].events = [{ type: 'array.mark_sorted', indices: [0, 1, 2, 3, 4, 5, 6, 7] }]

  const data = table.map(s => s ? s.charCodeAt(0) : 0)
  return { algorithm: 'hash_table', complexity: { time: { best: 'O(1)', average: 'O(1)', worst: 'O(n)' }, space: 'O(n)' }, presentation: { engine: 'scene', module: 'array' }, initialState: { type: 'array', data }, steps: steps as AnimationScript['steps'] }
}
