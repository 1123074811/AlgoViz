...
export function generateUnionFind(): AnimationScript {
  ...
  function find(x: number): number {
    if (parent[x] !== x) {
      ...
      steps[steps.length - 1].events = [{ type: 'tree.visit', nodeId: String(x) }]
  ...
  function union(x: number, y: number) {
    ...
    if (px !== py) {
      ...
      steps[steps.length - 1].events = [{ type: 'tree.visit', nodeId: String(px) }]
      parent[px] = py
    }
    ...
    steps[steps.length - 1].events = [{ type: 'tree.visit', nodeId: String(py) }]
  }
